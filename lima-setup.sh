#!/bin/bash
#
# Lima environment setup script for development with Fedora
# This script creates and starts a Lima VM for development
...
if [[ $CLEANUP -eq 1 ]]; then
  log_info "=== Cleaning up Youki (Fedora) Development Environment ==="
...
log_info "=== Youki (Fedora) Development Environment Setup ==="
log_info "Setting up Lima VM development with Fedora..."
...
# Lima configuration development environment with Fedora
...
if limactl list --json | grep -q "\"name\":\"$VM_NAME\""; then
  if [[ $FORCE_RECREATE -eq 1 ]]; then
    ...
    log_info "(Fedora) dev environment ready: limactl shell $VM_NAME"
    exit 0
  fi
fi
...
log_info "(Fedora) development environment '$VM_NAME' is ready!"
