...
impl HostExecutor for LibkrunExecutor {
    fn modify_spec(&self, spec: Spec) -> Result<Spec, ExecutorError> {
        ...
        validate_kvm().map_err(|e| ExecutorError::Other(format!("validate kvm error in host executor: {e}")))?;
...
pub fn write_krun_config(rootfs: &Path, json_spec: &str) -> Result<(), KrunError> {
    let mut root = Root::open(rootfs)
        .map_err(|e| KrunError::Other(format!("failed to open rootfs {e}")))?;
