...
fn check_recursive_readonly() -> TestResult {
    ...
    check_recursive(
        mount_options,
        None,
        |_bundle_path: &std::path::Path, _dir_path: &std::path::Path, subdir: &std::path::Path| {
            std::fs::write(subdir.join("bar"), b"bar\n")
...
fn check_recursive_readwrite() -> TestResult {
    ...
    check_recursive(
        ...
        |_bundle_path: &std::path::Path,
         _dir_path: &std::path::Path,
         subdir_path: &std::path::Path| {
            std::fs::write(subdir_path.join("bar"), b"bar\n").unwrap();
            Ok(())
        },
    )
}
...
fn check_rbind_ro_is_readonly_but_not_recursively() -> TestResult {
    ...
    check_recursive(
        ...
        |_bundle_path: &std::path::Path,
         dir_path: &std::path::Path,
         subdir_path: &std::path::Path| {
            std::fs::write(dir_path.join("bar"), b"bar\n").unwrap();
            std::fs::write(subdir_path.join("bar"), b"bar\n").unwrap();
            Ok(())
        },
    )
