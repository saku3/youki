...
fn create_spec(propagation: String) -> Result<Spec> {
    ...
    let sys_admin_capability: HashSet<Capability> = HashSet::from([Capability::SysAdmin]);

    let capabilities = LinuxCapabilitiesBuilder::default()
        .bounding(sys_admin_capability.clone())
        .effective(sys_admin_capability.clone())
        .inheritable(sys_admin_capability.clone())
        .permitted(sys_admin_capability.clone())
        .ambient(sys_admin_capability.clone())
        .build()
