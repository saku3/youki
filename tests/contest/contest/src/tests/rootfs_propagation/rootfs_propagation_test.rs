...
fn create_spec(propagation: String) -> Result<Spec> {
    ...
    let capabilities = LinuxCapabilitiesBuilder::default()
        .bounding([Capability::SysAdmin].into())
        .effective([Capability::SysAdmin].into())
        .inheritable([Capability::SysAdmin].into())
        .permitted([Capability::SysAdmin].into())
        .ambient([Capability::SysAdmin].into())
        .build()
