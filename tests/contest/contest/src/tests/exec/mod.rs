...
pub fn get_exec_test() -> TestGroup {
    ...
    let _cgroup_test = Test::new("cgroup_test", Box::new(cgroup_test::cgroup_test));
    let _no_capabilities_test = Test::new(
        "no_capabilities_test",
        Box::new(cap_test::get_test_no_capabilities),
    );
    let _new_privileges_test = Test::new(
        "new_privileges_test",
        Box::new(cap_test::get_test_new_privileges),
    );
    let _some_capabilities_test = Test::new(
        "some_capabilities_test",
        Box::new(cap_test::get_test_some_capabilities),
    );
    let _capabilities_by_flag_test_case1 = Test::new(
        "capabilities_by_flag_test_case1",
        Box::new(cap_test::get_test_capabilities_by_flag_case1),
    );
    let _capabilities_by_flag_test_case2 = Test::new(
