...
@test "runc run (cgroup v2 resources.unified override)" {
  ...
  echo "debug: running container" >&2
  runc run -d --console-socket "$CONSOLE_SOCKET" test_cgroups_unified
  echo "debug: runc run status = $status" >&2
  [ "$status" -eq 0 ]

  echo "debug: checking memory.min" >&2
  runc exec test_cgroups_unified cat /sys/fs/cgroup/memory.min
  echo "memory.min = $output" >&2
  [ "$status" -eq 0 ]
  [ "$output" = '131072' ]

  echo "debug: checking memory.max" >&2
  runc exec test_cgroups_unified cat /sys/fs/cgroup/memory.max
  echo "memory.max = $output" >&2
  [ "$status" -eq 0 ]
  [ "$output" = '10485760' ]

  echo "debug: checking pids.max" >&2
  runc exec test_cgroups_unified cat /sys/fs/cgroup/pids.max
  echo "pids.max = $output" >&2
  [ "$status" -eq 0 ]
  [ "$output" = '42' ]

  echo "debug: checking systemd value" >&2
  check_systemd_value "TasksMax" 42

  echo "debug: checking cpu quota" >&2
  check_cpu_quota 5000 50000

  echo "debug: checking cpu weight" >&2
  check_cpu_weight 42
}
