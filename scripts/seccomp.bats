...
@test "runc run [seccomp defaultErrnoRet=ENXIO]" {
  ...
  echo "==> Compiling test binary: ${TESTDATA}/${TEST_NAME}.c"
  gcc -static -o rootfs/seccomp_test2 "${TESTDATA}/${TEST_NAME}.c"
  if [ $? -ne 0 ]; then
    echo "Failed to compile ${TESTDATA}/${TEST_NAME}.c"
    exit 1
  fi

  echo "==> Contents of seccomp policy:"
  cat "${TESTDATA}/${TEST_NAME}.json"

  echo "==> Updating config with seccomp policy"
  update_config ".linux.seccomp = $(<"${TESTDATA}/${TEST_NAME}.json")"

  echo "==> Setting process args to /seccomp_test2"
  update_config '.process.args = ["/seccomp_test2"]'

  echo "==> Running runc"
  runc run test_busybox
  echo "runc exit status: $status"

  if [ "$status" -ne 0 ]; then
    echo "Test failed with status $status"
    echo "==> runc state (if exists):"
    runc state test_busybox || true
    echo "==> config.json snapshot:"
